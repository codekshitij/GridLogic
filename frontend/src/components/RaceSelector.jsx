...
const RaceSelector = ({ selectedYear, onYearChange, selectedGP, onGPChange }) => {
  ...
  useEffect(() => {
    ...
    const fetchRaces = async () => {
      try {
        const { data } = await axios.get(`http://127.0.0.1:8000/races/${selectedYear}`);
      ...
      <div className="max-w-[1600px] mx-auto flex justify-between items-center">
        ...
        <div className="flex gap-4">
          ...
          <select 
            ...
            className="bg-[#111] text-xs font-bold p-2 rounded border border-white/10"
          >
            {availableGPs.map(gp => <option key={gp} value={gp}>{gp}</option>)}
          </select>
        </div>
      </div>
    </nav>
